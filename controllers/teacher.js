...
const getSingle = async (req, res, next) => {
    ...
    const result = await mongodb.getDatabase().db().collection('teachers').find({ _id: userId });
    result.toArray().then((lists) => {
        res.setHeader('Content-Type', 'application/json');
...
const updateTeacher = async (req, res, next) => {
    ...
    const response = await mongodb.getDatabase().db().collection('teachers').replaceOne({ _id: userId }, teacher);
...
const deleteTeacher = async (req, res, next) => {
    const userId = new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db().collection('teachers').deleteOne({ _id: userId });
